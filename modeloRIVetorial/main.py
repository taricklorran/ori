...
#diretorio = r"C:\Users\lorra\Documents\UFU\modeloRIVetorial\\"
...
def gerar_indice_invertido(base_documentos):
    ...
    for indiceDocumento, nome_documento in enumerate(base_documentos, start=1):
        ...
        arquivo = lerDocumento(nome_documento)
...
def guardarArquivoPeso(dicTfIdf):
    with open("pesos.txt", "w") as arquivoPeso:
...
def guardarArquivoRespostaConsulta(qtdDocs, similaridades):
    with open("resposta.txt", "w") as arquivoSimilaridades:
...
def main():
    ...
    base_documentos = lerDocumento(baseDoc)
